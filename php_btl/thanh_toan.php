...
if (!isset($_SESSION["giohang"])) $_SESSION["giohang"]=[];
    // $div_a = $document->getElementById("oppcart");
    // $div_b = $document->getElementById("table");
...
if(isset($_POST["dat_hang"])){
    ...
    $ngay_het_han = date('Y-m-d', strtotime($ngay_thue . ' + ' . $day . ' days'));
    insert_don_hang($ho_ten,$ngay_thue,$ngay_het_han,$tinh_trang_thanh_toan,$tinh_trang_don_hang, $day);
    insert_khach_hang($ho_ten,$dia_chi,$phone,$mail);
    unset($_SESSION['giohang']);
    $_SESSION["giohang"]=[];
    header("Location: thanh_toan.php");
}
...
function showgiohang(){
    if(isset($_SESSION["giohang"])&&(is_array($_SESSION["giohang"]))){
        if(sizeof($_SESSION["giohang"])>0){
            echo'
            <div class="cart_dis_left col-md-8">
                <table class="table table-bordered table-hover" name="table_info" id="table">
                    <thead>
                        <tr>
                            <th scope="col">Ảnh</th>
            ...
            for( $i= 0; $i < sizeof($_SESSION["giohang"]); $i++){
                $tong = (int)$_SESSION["giohang"][$i][3] * $_SESSION["giohang"][$i][4];
                echo'<tr>
                        <td><img src=" '.$_SESSION['giohang'][$i][1].' "  width="30%" ></td>
                        <td>'.$_SESSION['giohang'][$i][2].' - '.$_SESSION['giohang'][$i][5].'</td>
                        ...
                        <td>'.$tong.' đ</td>
                        <th>
                        <a href="thanh_toan.php?xoa='.$i.'">Xóa</a>
                        </th>
                    </tr>';
            }
            echo '
                ...
                <div class="set_cart" style="margin:10px 0 10px 0; width:100%">
                    <div id="form-button">
                        <a href="../php_btl/sanpham.php">Thêm sản phẩm</a>
                    </div>
                    <div id="form-button">
                        <a href="../php_btl/thanh_toan.php?del=1">Xóa giỏ hàng</a>
                    </div>
                </div>
            </div>';
        }else{
            echo '
            <div class="cart_dis_left_emty col-md-8">
                    <div class="oppcart" id="oppcart">
                        <h3>Opps!!!</h3>
                        <svg width="250px" height="140px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 429.07 230.39">
                            <defs>
                            <style>
                                .cls-1 {
                                fill: #4268ae;
                        ...
                        <a class="btn btn-outline-warning button-cart" href="../php_btl/sanpham.php" role="button" >Keep Shopping</a>
                    </div> 
            </div>';
        }
    }
        ...
        if(sizeof($_SESSION["giohang"])>0){
            ...
            for( $i= 0; $i < sizeof($_SESSION["giohang"]); $i++){
                ...
                <div class="pay_detail">
                    <h6>'.$_SESSION['giohang'][$i][2].' - '.$_SESSION['giohang'][$i][5].'</h6>
                    <h6>x '.$_SESSION['giohang'][$i][4].'</h6>
                </div>';
            }
            echo '
            </div>
...
function get_ma_mau($size) {
    ...
    if ($result->num_rows > 0) {
        return $result->fetch_assoc()['ma_mau'] ?? null;
    }
    
    $conn->close();
}

function insert_khach_hang($ho_ten, $dia_chi, $phone, $mail) {
    connect();
    $conn = conn();
    $stmt = $conn->prepare ("INSERT INTO khach_hang(ho_ten, dia_chi, sdt, email) VALUES(?,?,?,?)");
    $stmt->bind_param("ssis", $ho_ten, $dia_chi, $phone, $mail);
    $stmt->execute();
    $stmt->close();
    $conn->close();
}
function insert_don_hang($ho_ten, $ngay_thue, $ngay_het_han, $tinh_trang_thanh_toan, $tinh_trang_don_hang, $day) {
    connect();
    $conn = conn();

    $stmt = $conn->prepare("INSERT INTO don_hang(ma_mau, ho_ten, ten_mau, so_luong, ngay_thue, ngay_het_han, gia, tinh_trang_thanh_toan, tinh_trang_don_hang) VALUES(?,?,?,?,?,?,?,?,?)");

    if (sizeof($_SESSION["giohang"]) > 0) {
        // Bind các tham số ngoài vòng lặp
        $stmt->bind_param("sssisssss", $ma_mau, $ho_ten, $ten_mau, $so_luong, $ngay_thue, $ngay_het_han, $tong, $tinh_trang_thanh_toan, $tinh_trang_don_hang);

        for ($i = 0; $i < sizeof($_SESSION["giohang"]); $i++) {
            // $product = [$ma_mau, $anh_mau, $ten_mau, $gia, $so_luong, $size];
            $ma_mau = $_SESSION['giohang'][$i][0];
            $ten_mau = $_SESSION['giohang'][$i][2];
            $so_luong = $_SESSION['giohang'][$i][4];
            $tong = (int)$_SESSION['giohang'][$i][3] * $so_luong * $day;
            ...
            try {
                ...
                die('<strong style="color: red;">Không có Mẫu này!</strong>');
            }
        }

        // Đóng câu truy vấn sau khi hoàn thành vòng lặp
        $stmt->close();
    } else {
        die('<strong style="color: red;">Cảnh báo: Không có mẫu nào trong giỏ hàng!</strong>');
    }
...
<body>
    ...
    <main>
        <form id="cart-form" action="thanh_toan.php" method="POST">
            <div class="container-cart row" >

                    <?php showgiohang();?>

                <div class="cart_dis_right col-md-3" style="margin-left: 10px">
        ...
        <!-- dat hang -->
            <div class="container " style=" width: 100%; margin-top:20px; border: 1px #dedede solid; border-radius: 10px; padding-top: 10px; padding-bottom: 10px;">
                <h2 style="border: 1px #dedede dashed; border-radius: 10px; padding: 10px;">Chi tiết đặt hàng</h2>
                <div class="dat_hang">
                    <form class="row g-3 needs-validation" action="../php_btl/thanh_toan.php" method="POST">
                        <div class="col-md-4">
                            <label for="validationCustom01" class="form-label">Họ tên</label>
                            <input type="text" class="form-control" id="validationCustom01" name="ho_ten" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control" id="validationCustom02" name="dia_chi" required>
                            <div class="invalid-feedback">
                        ...
                        <div class="col-md-6">
                            <label for="validationCustom03" class="form-label">Số ngày mượn</label>
                            <input type="number" class="form-control" id="day" name="day" value="1" min="1" max="99" required>
                            <div class="invalid-feedback">
                        ...
                        <label for="validationCustomUsername" class="form-label">Email</label>
                            ...
                            <input type="text" class="form-control" id="validationCustomUsername" name="mail" aria-describedby="inputGroupPrepend" required>
                            <div class="invalid-feedback">
                        ...
                        <div class="col-12">
                            <button class="btn btn-primary" type="submit" name = "dat_hang">Đặt hàng</button>
                        </div>
                    </form>
                </div>
            </div>
